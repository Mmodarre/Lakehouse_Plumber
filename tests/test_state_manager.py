...
from lhp.core.state_manager import FileState, ProjectState, StateManager
...
class TestStateManagerInitialization:
    ...
    def test_init_with_existing_state_file(self, caplog):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                "Initialized StateManager with service-based architecture"
                in caplog.text
            )
...
class TestStateManagerSaveState:
    ...
    def test_save_state_error(self, caplog):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Atomic save uses tempfile.mkstemp then os.fdopen + os.replace;
            # simulate a permission failure at the tempfile creation step.
            with patch(
...
class TestStateManagerTrackGeneratedFile:
    ...
    def test_track_generated_file_absolute_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(
                state_manager, "calculate_checksum", return_value="mock_checksum"
            ):
                state_manager.track_generated_file(
                    external_generated,
                    external_source,
...
""")
            ...
            init_file_relative = (
                "generated/customer_pipeline/custom_python_functions/__init__.py"
            )
            copied_file_relative = "generated/customer_pipeline/custom_python_functions/customer_cleaner.py"

            assert (
                main_file_relative in tracked_files
            ), f"Main file should be tracked: {tracked_files}"
            assert (
                init_file_relative in tracked_files
            ), f"__init__.py should be tracked: {tracked_files}"
            assert (
                copied_file_relative in tracked_files
            ), f"Copied Python file should be tracked: {tracked_files}"
...
class TestStateManagerFilesNeedingGeneration:
    ...
    def test_get_files_needing_generation_with_pipeline_filter(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(
                state_manager, "find_stale_files", return_value=[file_state2]
            ):
                with patch.object(
                    state_manager, "find_new_yaml_files", return_value=[]
                ):
                    result = state_manager.get_files_needing_generation(
                        "dev", "target_pipeline"
                    )
...
class TestStateManagerCleanupOperations:
    ...
    def test_cleanup_empty_directories(self, caplog):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state_manager._state.environments["dev"] = {
                "generated/pipeline1/test.py": file_state
            }
...
""")
            ...
            source_yaml_path = (
                project_root / "pipelines" / "data_pipeline" / "process_data.yaml"
            )
            ...
            context = {
                ...
                "spec_dir": project_root,
                "flowgroup": FlowGroup(
                    pipeline="data_pipeline", flowgroup="process_data", actions=[]
                ),
                "state_manager": state_manager,
            ...
            assert (
                len(tracked_files) == 3
            ), f"Should track 3 files, got: {list(tracked_files.keys())}"
