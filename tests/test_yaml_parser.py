...
class TestYAMLParserFlowgroupMethods:
    ...
    def test_parse_template_success(self):
        ...
        template_data = {
            "name": "test_template",
            "version": "1.0",
            "parameters": [{"name": "table_name", "required": True}],
            "actions": [
                {
...
""")
        ...
        try:
            ...
            assert len(flowgroups) == 3
            assert all(
                fg.use_template == "TMPL003_parquet_ingestion" for fg in flowgroups
            )
            assert (
                flowgroups[0].template_parameters["target_table"] == "bronze_sap_brand"
            )
            assert flowgroups[1].template_parameters["target_table"] == "bronze_sap_cat"
            assert (
                flowgroups[2].template_parameters["target_table"]
                == "bronze_sap_carrier"
            )
        finally:
            yaml_file.unlink()
