...
class TestBundleFileSystemErrorHandling:
    ...
    def test_bundle_manager_handles_disk_full_errors(self):
        ...
        with patch("pathlib.Path.write_text", side_effect=OSError("No space left on device")):
            with pytest.raises(BundleResourceError) as exc_info:
                bundle_manager._create_new_resource_file("test_pipeline", ["test.py"], "dev")
    ...
    def test_bundle_manager_recovery_from_partial_file_corruption(self):
        """Should recover from partial file corruption."""
        bundle_manager = BundleManager(self.project_root)
        
        # Create partially corrupted YAML file
        resources_dir = self.project_root / "resources"
        resources_dir.mkdir()
        corrupted_file = resources_dir / "test.yml"
        corrupted_file.write_text("""resources:
  pipelines:
    test:
      libraries:
        - notebook:
...
invalid yaml content""")
        ...
        notebooks = bundle_manager._get_notebook_paths_for_pipeline(
            self.project_root / "generated" / "test"
        )
        # Should return empty list due to corruption
        assert isinstance(notebooks, list)
...
class TestBundleErrorRecoveryMechanisms:
    ...
    def test_bundle_manager_graceful_degradation_on_yaml_errors(self):
        """Should fail fast when YAML processing fails."""
        bundle_manager = BundleManager(self.project_root)
        ...
        resources_dir = self.project_root / "resources"
        resources_dir.mkdir()
        corrupted_file = resources_dir / "test_pipeline.yml"
        ...
        error_msg = str(exc_info.value)
        assert "test_pipeline" in error_msg  # Should mention the pipeline
        assert ("yaml" in error_msg.lower() or "invalid" in error_msg.lower())  # Should mention the YAML issue
    ...
    def test_error_context_preservation_across_components(self):
        """Should preserve error context across different components."""
        bundle_manager = BundleManager(self.project_root)
        
        # Create generated directory with pipeline files to reach YAML processing
        generated_dir = self.project_root / "generated"
        pipeline_dir = generated_dir / "test_pipeline"
        pipeline_dir.mkdir(parents=True)
        (pipeline_dir / "test.py").write_text("# Test notebook")
        
        # Create existing resource file to trigger YAML processing
        resources_dir = self.project_root / "resources"
        resources_dir.mkdir()
...
""")
        ...
        with patch('lhp.bundle.yaml_processor.YAMLProcessor.extract_notebook_paths') as mock_extract:
            mock_extract.side_effect = YAMLParsingError("Original YAML error", context="test.yml:15")
            ...
            error_msg = str(exc_info.value)
            assert "Original YAML error" in error_msg
            assert "test.yml:15" in error_msg
...
line 5""")
    ...
    def test_bundle_error_includes_resolution_suggestions(self):
        ...
        with patch('pathlib.Path.mkdir', side_effect=PermissionError("Permission denied")):
            with pytest.raises(BundleResourceError) as exc_info:
                bundle_manager._create_new_resource_file("test", ["test.py"], "dev")
    ...
    def test_error_logging_provides_debug_information(self):
        ...
        with patch('pathlib.Path.exists', side_effect=OSError("Unexpected error")):
            with pytest.raises(BundleResourceError) as exc_info:
                bundle_manager._get_pipeline_directories(self.project_root / "generated")
