...
in ``lhp/core/codegen/operational_metadata/metadata.py``. This branch rewrites
``F.func()`` expressions to bare ``func()`` calls when a
``from pyspark.sql.functions import *`` wildcard is present, and otherwise
leaves the ``F.`` prefix intact. A regression here produces ``NameError`` in
generated pipelines.
