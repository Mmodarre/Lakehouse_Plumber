...
from lhp.core.loaders.external_file_loader import (
    is_file_path,
...
class TestResolveExternalFilePath:
    ...
    def test_resolves_relative_path(self, tmp_path):
        ...
        resolved = resolve_external_file_path(
            "schemas/customer.yaml", tmp_path, file_type="schema file"
        )
    ...
    def test_resolves_absolute_path(self, tmp_path):
        ...
        resolved = resolve_external_file_path(
            str(schema_file), tmp_path, file_type="schema file"
        )
    ...
    def test_resolves_subdirectories(self, tmp_path):
        ...
        # Resolve
        resolved = resolve_external_file_path(
            "schemas/silver/dimensions/customer.yaml", tmp_path, file_type="schema file"
        )
    ...
    def test_file_not_found_error(self, tmp_path):
        ...
        with pytest.raises(LHPError) as exc_info:
            resolve_external_file_path(
                "schemas/missing.yaml", tmp_path, file_type="schema file"
            )
    ...
    def test_handles_path_object(self, tmp_path):
        ...
        resolved = resolve_external_file_path(
            Path("customer.yaml"), tmp_path, file_type="schema file"
        )
...
class TestLoadExternalFileText:
    ...
    def test_loads_sql_file(self, tmp_path):
        ...
        loaded = load_external_file_text("query.sql", tmp_path, file_type="SQL file")
    ...
    def test_loads_ddl_file(self, tmp_path):
        ...
        loaded = load_external_file_text("schema.ddl", tmp_path, file_type="DDL file")
    ...
    def test_handles_encoding(self, tmp_path):
        ...
        loaded = load_external_file_text(
            "data.txt", tmp_path, file_type="text file", encoding="utf-8"
        )
    ...
    def test_loads_from_subdirectory(self, tmp_path):
        ...
        loaded = load_external_file_text(
            "sql/transforms/customer_transform.sql", tmp_path, file_type="SQL file"
        )
    ...
    def test_file_not_found_error(self, tmp_path):
        ...
        with pytest.raises(LHPError) as exc_info:
            load_external_file_text("missing.sql", tmp_path, file_type="SQL file")
...
class TestIntegration:
    ...
    def test_schema_file_in_nested_directory(self, tmp_path):
        ...
        schema_path = (
            tmp_path / "schemas" / "silver" / "dimensions" / "brand_schema.yaml"
        )
...
"""
        ...
        # Test resolution
        resolved = resolve_external_file_path(
            file_ref, tmp_path, file_type="schema file"
        )
        assert resolved.exists()

        # Test loading
        loaded = load_external_file_text(file_ref, tmp_path, file_type="schema file")
    ...
    def test_sql_file_loading(self, tmp_path):
        ...
        loaded = load_external_file_text(file_ref, tmp_path, file_type="SQL file")
...
created_at TIMESTAMP"""
        ...
        loaded = load_external_file_text(
            file_ref, tmp_path, file_type="table schema file"
        )
