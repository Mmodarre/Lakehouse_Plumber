...
class TestLoadOperationalMetadata:
    ...
    def test_delta_load_with_operational_metadata(self):
        ...
        action = Action(
            name="load_delta",
            type=ActionType.LOAD,
            source={"type": "delta", "table": "source.customers"},
            target="v_customers",
    ...
    def test_jdbc_load_with_operational_metadata(self):
        ...
        action = Action(
            name="load_jdbc",
            type=ActionType.LOAD,
            source={"type": "jdbc", "table": "customers", "connection": "my_db"},
            target="v_customers",
            operational_metadata=["_ingestion_timestamp", "_source_table"],
        )
    ...
    def test_python_load_with_operational_metadata(self, tmp_path):
        ...
        context = {**self.context, "spec_dir": tmp_path}
    ...
    def test_operational_metadata_disabled(self):
        ...
        action = Action(
            name="load_files",
            type=ActionType.LOAD,
            source={"type": "cloudfiles", "path": "/data/files/*.json"},
            target="v_raw_data",
...
age: int
        ...
        assert (
            'withColumn("_ingestion_timestamp", F.col("_ingestion_timestamp").cast('
            not in code
        )
