...
def test_write_creates_file_when_absent(tmp_path: Path) -> None:
    status = cr.write_routing_block(tmp_path)
    assert status == "created"
    text = _read(tmp_path)
    assert cr.ROUTING_START in text
    assert cr.ROUTING_END in text
    assert "Lakehouse Plumber project" in text


def test_write_is_idempotent(tmp_path: Path) -> None:
    assert cr.write_routing_block(tmp_path) == "created"
    # A second write detects the block is already current and changes nothing.
    assert cr.write_routing_block(tmp_path) == "unchanged"
    text = _read(tmp_path)
    # Exactly one block — not appended twice.
    assert text.count(cr.ROUTING_START) == 1
    assert text.count(cr.ROUTING_END) == 1
...
def test_write_appends_to_existing_file_without_block(tmp_path: Path) -> None:
    ...
    assert cr.ROUTING_START in text
    assert text.index("My Project") < text.index(cr.ROUTING_START)


def test_write_replaces_stale_block_in_place(tmp_path: Path) -> None:
    head = "# My Project\n\nIntro.\n\n"
    tail = "\n\n## My own footer\nKeep me.\n"
    stale = f"{cr.ROUTING_START}\nOLD CONTENT\n{cr.ROUTING_END}"
...
def test_remove_deletes_block_and_preserves_surrounding(tmp_path: Path) -> None:
    ...
    cr.write_routing_block(tmp_path)

    status = cr.remove_routing_block(tmp_path)
    assert status == "removed"
    text = _read(tmp_path)
    assert "Intro." in text
    assert cr.ROUTING_START not in text
...
def test_remove_is_absent_when_no_file(tmp_path: Path) -> None:
    assert cr.remove_routing_block(tmp_path) == "absent"
