...
class TestPresetTemplateCombination:
    ...
    def test_template_with_missing_preset_raises_error(self):
        ...
        ``LHPError`` through ``collect_response`` rather than returning a
        ``not response.success`` DTO. Intent (an unknown preset is
        rejected with a message naming the offending preset) is preserved.
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self._create_project_directories(project_root)
