...
class TestCodeGeneratorDetermineActionSubtype:
    ...
    def test_determine_action_subtype_load_string_source(self, code_generator):
        ...
        action = Action(
            name="load_data", type=ActionType.LOAD, source="SELECT * FROM table"
        )
    ...
    def test_determine_action_subtype_transform_with_type(self, code_generator):
        ...
        action = Action(
            name="transform_data", type=ActionType.TRANSFORM, transform_type="sql"
        )
    ...
    def test_determine_action_subtype_transform_no_type(self, code_generator):
        ...
        action = Action(name="transform_data", type=ActionType.TRANSFORM)
    ...
    def test_determine_action_subtype_write_no_target(self, code_generator):
        ...
        action = Action(name="write_data", type=ActionType.WRITE)
    ...
    def test_determine_action_subtype_test_with_type(self, code_generator):
        ...
        action = Action(name="test_data", type=ActionType.TEST, test_type="row_count")
    ...
    def test_determine_action_subtype_test_no_type(self, code_generator):
        ...
        action = Action(name="test_data", type=ActionType.TEST)
...
class TestCodeGeneratorGroupWriteActionsByTarget:
    ...
    def test_group_write_actions_table_only(self, code_generator):
        ...
        action = Action(
            name="write_table1", type=ActionType.WRITE, write_target={"table": "table1"}
        )
    ...
    def test_group_write_actions_name_fallback(self, code_generator):
        ...
        action = Action(name="write_table1", type=ActionType.WRITE, write_target={})
...
class TestCodeGeneratorCreateCombinedWriteAction:
    ...
    def test_create_combined_write_action_single_action(self, code_generator):
        ...
        combined = code_generator.create_combined_write_action(
            [action], "test_cat.test_sch.table1"
        )
...
class TestCodeGeneratorExtractSourceViewsFromAction:
    ...
    def test_extract_source_views_dict_with_table_only(self, code_generator):
        ...
        result = code_generator._extract_source_views_from_action({"table": "table1"})

        assert result == ["table1"]

    def test_extract_source_views_dict_with_view(self, code_generator):
        """Test extracting source views from dict with view key."""
        result = code_generator._extract_source_views_from_action({"view": "v_view1"})
...
class TestCodeGeneratorGenerateActionSections:
    ...
    def test_generate_action_sections_with_tests_included(self, code_generator):
        ...
        flowgroup = FlowGroup(
            ...
            actions=[Action(name="test1", type=ActionType.TEST, test_type="row_count")],
        )
        ...
        mock_generator.get_pre_pipeline_statements = Mock(return_value=[])
        code_generator.action_registry.get_generator.return_value = mock_generator

        code_generator.dependency_resolver.resolve_dependencies.return_value = (
            flowgroup.actions
        )
...
class TestCodeGeneratorDQSectionPartitioning:
    ...
    def _make_mock_generator(self, code_generator, return_code="# generated"):
        ...
        mock_gen.get_pre_pipeline_statements = Mock(return_value=[])
        code_generator.action_registry.get_generator.return_value = mock_gen
        return mock_gen

    def test_dq_only_flowgroup_has_dq_section_no_transform_section(
        self, code_generator
    ):
