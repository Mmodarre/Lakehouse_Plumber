...
class TestVersionEnforcement:
    """Test version enforcement at facade construction time.

    These tests exercise the construction path
    :meth:`LakehousePlumberApplicationFacade.for_project`, which builds the
    underlying orchestrator via ``build_facade_orchestrator``. Version
    enforcement happens inside the orchestrator constructor, so any
    LHPError surfaces through ``for_project`` exactly as it did from the
    legacy direct construction path.

    A couple of tests assert on ``project_config`` / ``enforce_version``,
    which are orchestrator-internal attributes. We access them via
    ``facade._orchestrator`` as a deliberate test-only accommodation:
    these are construction-time concerns and the facade does not (yet)
    surface them publicly. See note in each such test.
    """
...
""")
        ...
        assert facade._orchestrator.project_config.required_lhp_version is None
...
""")
        ...
        with patch('lhp.core.coordination.orchestrator.get_version', return_value='0.4.1'):
            facade = LakehousePlumberApplicationFacade.for_project(tmp_path)
            # Inspecting orchestrator-internal state (construction-time concern).
            assert facade._orchestrator.project_config.required_lhp_version == ">=0.4.0,<0.5.0"
...
""")
        ...
        with patch('lhp.core.coordination.orchestrator.get_version', return_value='0.4.1'):
            with pytest.raises(LHPError) as exc_info:
                LakehousePlumberApplicationFacade.for_project(tmp_path)
        ...
        with patch('lhp.core.coordination.orchestrator.get_version', return_value='0.4.1'):
            ...
            for bypass_value in ["1", "true", "yes", "TRUE", "YES"]:
                with patch.dict(os.environ, {"LHP_IGNORE_VERSION": bypass_value}):
                    ...
                    facade = LakehousePlumberApplicationFacade.for_project(tmp_path)
                    # Inspecting orchestrator-internal state (construction-time concern).
                    assert facade._orchestrator.project_config.required_lhp_version == ">=0.5.0,<0.6.0"
...
""")
        ...
        with patch('lhp.core.coordination.orchestrator.get_version', return_value='0.4.1'):
            ...
            facade = LakehousePlumberApplicationFacade.for_project(tmp_path, enforce_version=False)
            # Inspecting orchestrator-internal state (construction-time concern).
            assert facade._orchestrator.project_config.required_lhp_version == ">=0.5.0,<0.6.0"
            assert facade._orchestrator.enforce_version is False
...
""")
    ...
    def test_no_project_config_skips_check(self, tmp_path):
        ...
        with patch('lhp.core.coordination.orchestrator.get_version', return_value='0.4.1'):
            facade = LakehousePlumberApplicationFacade.for_project(tmp_path)
            # Inspecting orchestrator-internal state (construction-time concern).
            assert facade._orchestrator.project_config is None
