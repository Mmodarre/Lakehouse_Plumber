...
| Title carries command/env/elapsed/pipeline count (`..._title_contains_header_fields`, `..._with_records_shows_overall_progress` `pipelines=1`) | `test_model.py::test_run_header_construct_and_frozen` (RunHeader holds command/env/pipeline_count) + `test_factory.py` render-path tests build a `RunHeader` end to end |
