...
from
  (
    ...
    from
      acme_edw_prod.edw_silver.partsupp_dim
  ) ps
    join (
      select
        *,
      ...
      coalesce(cast(replace(`__END_AT`, '-W', '') as int),99999999) as `end_week_key`
      from
        acme_edw_prod.edw_silver.supplier_dim
    ) s
    ...
    join (
      ...
      from
        acme_edw_prod.edw_silver.part_dim
    ) p
...
@dp.table(
    name="acme_edw_prod.edw_gold.partsupp_modelled_fct",
    comment="Materialized view: partsupp_modelled_fct",
    table_properties={},
)
def partsupp_modelled_fct():
    """Write to acme_edw_prod.edw_gold.partsupp_modelled_fct from multiple sources"""
