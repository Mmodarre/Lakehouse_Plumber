...
FROM acme_edw_prod.edw_silver.customer_dim c
JOIN acme_edw_prod.edw_silver.orders_fct o ON c.customer_id = o.customer_id
  AND o.order_date >= c.__start_at
  AND (o.order_date < c.__end_at OR c.__end_at IS NULL)
JOIN acme_edw_prod.edw_silver.nation_dim n ON c.nation_id = n.nation_id
...
@dp.table(
    name="acme_edw_prod.edw_gold.customer_lifetime_value_mv",
    comment="Materialized view: customer_lifetime_value_mv",
    table_properties={},
)
def customer_lifetime_value_mv():
    """Write to acme_edw_prod.edw_gold.customer_lifetime_value_mv from multiple sources"""
