...
FROM acme_edw_prod.edw_gold.sales_summary_monthly_mv
"""
    )
...
FROM acme_edw_prod.edw_gold.revenue_by_region_mv
GROUP BY month
"""
    )
...
FROM acme_edw_prod.edw_gold.customer_lifetime_value_mv
"""
    )
...
FROM acme_edw_prod.edw_gold.product_performance_mv
GROUP BY month
"""
    )
...
@dp.table(
    name="acme_edw_prod.edw_gold.executive_dashboard_mv",
    comment="Materialized view: executive_dashboard_mv",
    table_properties={},
)
def executive_dashboard_mv():
    """Write to acme_edw_prod.edw_gold.executive_dashboard_mv from multiple sources"""
