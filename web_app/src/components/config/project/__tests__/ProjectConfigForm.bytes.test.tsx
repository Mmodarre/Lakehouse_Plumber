...
//
// These tests drive the REAL form (fields → mutate → yaml-doc → buffer)
// and assert the feature's core promise: an edit only ever touches the
...
describe('ProjectConfigForm — byte preservation', () => {
  it('FLAGSHIP: editing one scalar saves a body differing in exactly one line; comments survive', async () => {
    const { bufferContent } = serveProject(FIXTURE)
  ...
  it('a bare-null section (`monitoring:`) becomes a map on the first field write', async () => {
    ...
    const { bufferContent } = serveProject('name: acme\nmonitoring:\n')
    await renderProjectForm()
    const user = userEvent.setup()

    // Toggling monitoring off is the first field write on the bare-null section.
    await user.click(await screen.findByRole('switch', { name: 'Enabled' }))

    expect(bufferContent()).toBe('name: acme\nmonitoring:\n  enabled: false\n')
  })

  it('an empty lhp.yaml gains a document on first edit and saves only what was set', async () => {
    const { bufferContent } = serveProject('')
    await renderProjectForm()
    const user = userEvent.setup()
    ...
    expect(bufferContent()).toBe('name: fresh_project\n')
  })
})
