...
import type { IssueListItem } from '../components/validation/IssueList'
  ...
  return (
    <div className="space-y-3 px-3 py-2.5">
      {data.summary && Object.keys(data.summary).length > 0 && (
        <div>
          <h4 className="mb-1.5 text-2xs font-semibold uppercase tracking-[0.05em] text-muted-foreground">
            Summary
          </h4>
  ...
  return (
    <div className="h-full overflow-y-auto bg-background p-6">
      <div className="mx-auto w-full max-w-5xl space-y-4">
        <div>
          <h1 className="text-lg font-semibold text-foreground">Run history</h1>
          <p className="mt-1 text-xs text-muted-foreground">
            Recorded validate and generate runs, newest first. Select a run to
            inspect its issues and event frames.
          </p>
        </div>
...
function RunRow({
  ...
  return (
    <>
      <tr
        onClick={onToggle}
        className={cn('cursor-pointer transition-colors hover:bg-muted/50', selected && 'bg-muted/50')}
      >
        <td className="px-1.5 py-1.5">
          {/* Real button so the expand/collapse is keyboard-reachable —
              the row onClick is a mouse-convenience duplicate. */}
          <Button
            variant="ghost"
            size="icon-xs"
