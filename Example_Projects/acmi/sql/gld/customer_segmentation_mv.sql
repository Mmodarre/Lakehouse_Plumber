...
SELECT
    ...
    CASE
        ...
        AND monetary_score >= 2 THEN 'Potential Loyalists'
        WHEN recency_score >= 4
        AND frequency_score <= 1
        AND monetary_score <= 2 THEN 'New Customers'
        WHEN recency_score >= 3
        AND frequency_score >= 1
        AND monetary_score >= 1 THEN 'Promising'
        WHEN recency_score >= 2
        AND frequency_score >= 3
        AND monetary_score >= 3 THEN 'Need Attention'
        WHEN recency_score >= 2
        AND frequency_score >= 2
        AND monetary_score >= 2 THEN 'About to Sleep'
        WHEN recency_score <= 2
        AND frequency_score >= 2
        AND monetary_score >= 3 THEN 'At Risk'
        WHEN recency_score <= 1
        AND frequency_score >= 3
        AND monetary_score >= 4 THEN 'Cannot Lose Them'
        WHEN recency_score <= 1
        AND frequency_score <= 1
        AND monetary_score <= 2 THEN 'Hibernating'
        WHEN recency_score <= 1
        AND frequency_score <= 2
        AND monetary_score <= 2 THEN 'Lost'
        ELSE 'Others'
    ...
    CASE
        WHEN recency_score >= 4
        AND frequency_score >= 4
        AND monetary_score >= 4 THEN 1 -- Champions
        WHEN recency_score <= 1
        AND frequency_score >= 3
        AND monetary_score >= 4 THEN 2 -- Cannot Lose Them
        WHEN recency_score <= 2
        AND frequency_score >= 2
        AND monetary_score >= 3 THEN 3 -- At Risk
        WHEN recency_score >= 3
        AND frequency_score >= 3
        AND monetary_score >= 3 THEN 4 -- Loyal Customers
        WHEN recency_score >= 2
        AND frequency_score >= 3
        AND monetary_score >= 3 THEN 5 -- Need Attention
        ELSE 6
