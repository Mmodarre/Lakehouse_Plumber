...
# FlowGroup: lineitem_bronze_europe
...
FLOWGROUP_ID = "lineitem_bronze_europe"
...
@dlt.view()
def v_lineitem_europe_raw():
    """Load lineitem table from raw schema"""
    df = spark.readStream.table("acme_edw_tst.edw_raw.lineitem_europe_raw")
...
def v_lineitem_europe_bronze_cleaned():
...
def v_lineitem_europe_bronze_DQE():
    """Apply data quality checks to lineitem"""
    df = spark.readStream.table("v_lineitem_europe_bronze_cleaned")
...
@dlt.append_flow(
    ...
    name="f_lineitem_europe_bronze",
    comment="Append flow to acme_edw_tst.edw_bronze.lineitem",
)
def f_lineitem_europe_bronze():
    """Append flow to acme_edw_tst.edw_bronze.lineitem"""
    # Streaming flow
    df = spark.readStream.table("v_lineitem_europe_bronze_DQE")
