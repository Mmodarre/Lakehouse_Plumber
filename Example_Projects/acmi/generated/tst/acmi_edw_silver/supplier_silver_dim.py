...
def next_snapshot_and_version(
    ...
    if latest_snapshot_version is None:
        df = spark.sql(
            """
            SELECT * FROM acme_edw_tst.edw_bronze.supplier
            WHERE snapshot_id = (SELECT min(snapshot_id) FROM acme_edw_tst.edw_bronze.supplier)
        """
        )
        ...
        min_snapshot_id = (
            spark.sql(
                """
            SELECT min(snapshot_id) as min_id FROM acme_edw_tst.edw_bronze.supplier
        """
            )
            .collect()[0]
            .min_id
        ...
        next_snapshot_result = spark.sql(
            ...
            FROM acme_edw_tst.edw_bronze.supplier
...
dlt.create_streaming_table(
    name="acme_edw_tst.edw_silver.supplier_dim",
...
dlt.create_auto_cdc_from_snapshot_flow(
    target="acme_edw_tst.edw_silver.supplier_dim",
