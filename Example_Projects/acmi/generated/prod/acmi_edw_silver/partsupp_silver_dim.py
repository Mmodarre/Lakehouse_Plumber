...
def next_snapshot_and_version(
    ...
    if latest_snapshot_version is None:
        df = spark.sql(
            """
            SELECT * FROM acme_edw_prod.edw_bronze.partsupp
            WHERE snapshot_id = (SELECT min(snapshot_id) FROM acme_edw_prod.edw_bronze.partsupp)
        """
        )
        ...
        min_snapshot_id = (
            spark.sql(
                """
            SELECT min(snapshot_id) as min_id FROM acme_edw_prod.edw_bronze.partsupp
        """
            )
            .collect()[0]
            .min_id
        ...
        next_snapshot_result = spark.sql(
            ...
            FROM acme_edw_prod.edw_bronze.partsupp
