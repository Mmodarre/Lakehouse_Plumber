...
def next_snapshot_and_version(latest_snapshot_version: Optional[int]) -> Optional[Tuple[DataFrame, int]]:
    ...
    if latest_snapshot_version is None:
        df = spark.sql("""
            SELECT * FROM acme_edw_dev.edw_bronze.supplier
            WHERE snapshot_id = (SELECT min(snapshot_id) FROM acme_edw_dev.edw_bronze.supplier)
        """)

        min_snapshot_id = spark.sql("""
            SELECT min(snapshot_id) as min_id FROM acme_edw_dev.edw_bronze.supplier
        """).collect()[0].min_id
        ...
        next_snapshot_result = spark.sql(f"""
            ...
            FROM acme_edw_dev.edw_bronze.supplier
...
dlt.create_streaming_table(
    name="acme_edw_dev.edw_silver.supplier_dim",
