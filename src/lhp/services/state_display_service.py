...
class StateDisplayService:
    ...
    def get_tracked_files(
        self, env: str, pipeline: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if pipeline:
            ...
            for yaml_file in new_files:
                try:
                    ...
                    logger.debug(f"Could not make path relative for filtering: {e}")
                    continue
    ...
    def group_files_by_pipeline(self, files: List[Path]) -> Dict[str, List[Path]]:
        ...
        for file_path in files:
            try:
                ...
                logger.debug(f"Could not determine pipeline for {file_path}: {e}")
