...
| **CFG-067** | Invalid UC `tags_file` — non-mapping, missing/unknown top-level key, unsupported `version` (expected `1.0` or `1.0.0`), wrong-typed `table`/`tags`, or a `table:` that does not match the write target (the mismatch check is skipped under `--sandbox`) | Use the strict `version`/`table`/`tags` format; set `table:` to the write target's table name |
