...
- **Dependency analysis** (`lhp dag`): `source_function.parameters` are statically resolved — bound to the function's keyword-only args exactly as codegen applies them (function looked up at module top level by `function` name; `${token}` bytes preserved, never resolved at dag time) — so reads like `spark.read.table(f"{catalog}.{schema}.t")` over bound parameters resolve to edges. Same-file helper calls, parameter unions, return values and builtin collection folds resolve too; only genuinely runtime-only reads (env vars, external I/O) emit the advisory `LHP-DEP-002` (warning-only, never an error), aggregated per read site. Declare those edges with the additive `depends_on` action field (malformed entries → `LHP-VAL-063`); non-empty `depends_on` also suppresses the action's DEP advisories.
