...
```yaml
name: my_project
project_id: 3f1c9e4a-6b2d-4e8f-9a70-5c1d2e3f4a5b  # written by `lhp init`; telemetry only
version: "1.0"
...
Optional top-level `project_id:` key in `lhp.yaml`. `lhp init` writes a fresh UUID v4
(the same value it writes as `bundle.uuid` in `databricks.yml` when bundle support is
on). Read only by anonymous usage telemetry, never by code generation. LHP never edits
an existing `lhp.yaml` to add it.

| key | type | default | notes |
|-----|------|---------|-------|
| `project_id` | str (UUID v4) | absent | Opaque per-project id. Sent only as `sha256("lhp-project:" + value.strip().lower()).hexdigest()[:32]`. Safe to commit — that is the point: every developer's and every CI run's events roll up to one project. |

Resolution order when telemetry needs a project id: `lhp.yaml` `project_id` (source
`lhp_yaml`) → `databricks.yml` `bundle.uuid` (source `bundle_uuid`) → `lhp.yaml` `name`
(source `name_hash`, pseudonymous — the salt is public) → none. A non-string value fails
`ProjectConfig` validation like any other mistyped key.

- Source: `src/lhp/schemas/project.schema.json` (`project_id`); `src/lhp/models/_project.py:ProjectConfig`; resolver `src/lhp/telemetry/_identity.py:read_project_identity`; templates `src/lhp/templates/init/lhp.yaml.j2`, `src/lhp/templates/init_sample/lhp.yaml.j2`.
...
| switch | off when | scope |
|--------|----------|-------|
| `LHP_TELEMETRY` | `off` / `0` / `false` (case-insensitive) | process |
| `DO_NOT_TRACK` | set to anything but empty, `0`, `false`, `no` | process |
| `LHP_DISABLE_ANALYTICS` | same values as `DO_NOT_TRACK` | process (permanent alias) |
| `PYTEST_CURRENT_TEST` | set (pytest sets it itself) | any pytest run |
...
| var | effect |
|-----|--------|
| `LHP_TELEMETRY` | `off`/`0`/`false` = off; `log` = print the envelope to stderr, no spool, no send; anything else ignored |
| `DO_NOT_TRACK` | off |
...
constant) — see [`project_id`](#project_id-project-identity).

- Source: `src/lhp/telemetry/` (`_consent.py` order, `_paths.py` dirs + endpoint guard, `_spool.py` caps, `_sender.py` transport); CLI seam `src/lhp/cli/_telemetry_hook.py`; command `src/lhp/cli/commands/telemetry_command.py`; write failure → `LHP-IO-028`.
