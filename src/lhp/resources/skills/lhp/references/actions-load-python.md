...
- **Dependency analysis** statically extracts table reads from the copied Python (`spark.table`, `spark.read`/`readStream.table`, `spark.read.format("delta"|"iceberg"|"hive"|"unity_catalog").table`/`.load`, `spark.sql`, statically-resolvable names). For names it cannot resolve, declare edges with the additive `depends_on` action field (list of `catalog.schema.table` / `schema.table` refs; malformed → `LHP-VAL-063`).
