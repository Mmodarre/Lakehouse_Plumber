...
- **Dependency analysis** (`lhp dag`) statically extracts table reads from the copied Python (`spark.table`, `spark.read`/`readStream.table`, `spark.read.format("delta"|"iceberg"|"hive"|"unity_catalog").table`/`.load`, `spark.sql`, statically-resolvable names incl. string methods, f-strings over bound values, and `for`-loop unrolling over statically-foldable iterables incl. dict keys). YAML `source.parameters` is statically resolved — the dict is bound as the entry function's 2nd positional arg exactly as codegen passes it (`fn(spark, parameters)`; `function_name` defaults to `get_df`), so `parameters["k"]` / `parameters.get("k", default)` reads resolve with `${token}` bytes preserved. Resolution is inter-procedural within a file: same-file helper calls, function parameters (union across call sites), return values, `x or default`, and `list`/`tuple`/`sorted`/`set`/`dict.fromkeys` all fold. Genuinely runtime-only reads (env vars, external I/O) → advisory `LHP-DEP-002` (warning-only, never an error), aggregated per read site; declare those edges with the additive `depends_on` action field (list of `catalog.schema.table` / `schema.table` refs; malformed → `LHP-VAL-063`) — a non-empty `depends_on` also suppresses that action's DEP advisories.
