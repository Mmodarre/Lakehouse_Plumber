...
class StateCommand(BaseCommand):
    ...
    def execute(
        ...
        logger.debug(
            f"State request: env={env}, pipeline={pipeline}, "
            f"orphaned={orphaned}, stale={stale}, new={new}, cleanup={cleanup}, regen={regen}"
        )
        ...
        if not env:
            logger.debug("No environment specified, displaying overall stats")
        ...
        if orphaned:
            self._handle_orphaned_files_operation(
                service, env, pipeline, cleanup, dry_run
            )
            return
        ...
        logger.debug(f"Displaying comprehensive state view for env={env}")
        self._display_comprehensive_view(
            service, tracked_files, env, pipeline, project_root
        )
    ...
    def _handle_orphaned_files_operation(
        self,
        service: StateDisplayService,
        env: str,
    ...
    def _handle_stale_files_operation(
        self,
        service: StateDisplayService,
        env: str,
        pipeline: Optional[str],
        ...
        StateDisplayUtils.display_stale_files(
            stale_files, staleness_info, regen, dry_run
        )
    ...
    def _display_comprehensive_view(
        self,
        service: StateDisplayService,
        tracked_files,
        env: str,
        ...
        StateDisplayUtils.display_new_files_in_summary(
            new_files, project_root, env, by_pipeline
        )
