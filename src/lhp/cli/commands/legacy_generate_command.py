...
from ...core.orchestrator import ActionOrchestrator, GenerationAnalysis
...
class LegacyGenerateCommand(BaseCommand):
    """
    ...
    def execute(self, env: str, pipeline: Optional[str] = None, output: Optional[str] = None,
        ...
        # Smart generation analysis - delegate to orchestrator
        pipelines_needing_generation = self._analyze_generation_needs(
            pipelines_to_generate, env, state_manager, no_cleanup, force, include_tests, orchestrator
        )
    ...
    def _analyze_generation_needs(self, pipelines_to_generate: List[str], env: str,
                                 ...
                                 force: bool, include_tests: bool, 
                                 orchestrator: ActionOrchestrator) -> Dict[str, Dict]:
        """Analyze which pipelines need generation - now delegates to orchestrator for business logic."""
        ...
        analysis = orchestrator.analyze_generation_requirements(
            ...
            force=force,
            state_manager=state_manager if not no_cleanup else None
        )
        
        # Pure presentation logic - display results from orchestrator analysis
        self._display_generation_analysis(analysis, env, self.verbose)
        
        return analysis.pipelines_needing_generation
    
    def _display_generation_analysis(self, analysis: GenerationAnalysis, env: str, verbose: bool) -> None:
        """Display generation analysis results - pure presentation logic."""
        # Only show analysis if there's state management happening
        if analysis.detailed_staleness_info:
            click.echo(f"🔍 Analyzing changes in environment: {env}")
            
            # Show global dependency changes if any
            if analysis.has_global_changes:
                click.echo("🌍 Global dependency changes detected:")
                for change in analysis.global_changes:
                    click.echo(f"   • {change}")
                click.echo("   All files will be regenerated")
                return  # Early return for global changes
            
            # Show pipeline-by-pipeline analysis
            for pipeline_name, pipeline_info in analysis.pipelines_needing_generation.items():
                reason = analysis.get_generation_reason(pipeline_name)
                click.echo(f"   {pipeline_name}: {reason} file(s)")
                
                # Show detailed dependency changes for verbose mode
                if verbose and "stale" in pipeline_info and len(pipeline_info["stale"]) > 0:
                    for file_state in pipeline_info["stale"]:
                        file_path = file_state.generated_path
                        if file_path in analysis.detailed_staleness_info.get("files", {}):
                            file_info = analysis.detailed_staleness_info["files"][file_path]
                            click.echo(f"      • {file_path}:")
                            for detail in file_info["details"]:
                                click.echo(f"        - {detail}")
            
            # Show up-to-date pipelines
            for pipeline_name, file_count in analysis.pipelines_up_to_date.items():
                click.echo(f"   ✅ {pipeline_name}: {file_count} file(s) up-to-date")
            
            # Show generation context information
            if analysis.include_tests_context_applied:
            ...
            if not analysis.has_work_to_do():
                ...
                total_pipelines = len(analysis.pipelines_needing_generation) + len(analysis.pipelines_up_to_date)
                if len(analysis.pipelines_up_to_date) > 0:
                    click.echo(
                        f"⚡ Smart generation: processing {len(analysis.pipelines_needing_generation)}"
                        f"/{total_pipelines} pipelines"
                    )
        
        elif analysis.pipelines_needing_generation:
            # Force mode or no state tracking
            if any(info.get("reason") == "force" for info in analysis.pipelines_needing_generation.values()):
            ...
            if pipeline_orphaned:
                ...
                if not dry_run:
                    ...
                    for file_state in pipeline_orphaned:
                        ...
                        # Remove from state using proper encapsulation
                        state_manager.remove_generated_file(generated_path, env)
