...
class DQEParser:
    ...
    def parse_expectations(self, expectations: List[Dict]) -> Tuple[Dict, Dict, Dict]:
        ...
        for expectation in expectations:
            ...
            if not constraint:
                self.logger.warning(f"Expectation missing constraint/expression: {expectation}")
                continue
    ...
    def load_expectations_from_file(self, expectations_file: Path) -> List[Dict]:
        ...
        self.logger.info(f"Loaded {len(expectations)} expectations from {expectations_file}")
    ...
    def validate_expectations(self, expectations: List[Dict]) -> List[str]:
        ...
        for i, expectation in enumerate(expectations):
            ...
            if 'constraint' not in expectation and 'expression' not in expectation:
                errors.append(f"Expectation {i}: Missing 'constraint' or 'expression' field")
            ...
            if failure_action:
                ...
                if failure_action not in ['fail', 'drop', 'warn']:
                    errors.append(f"Expectation {i}: Invalid failureAction '{failure_action}'. Must be one of: fail, drop, warn")
            else:
                # Validate type field
                valid_types = ['expect', 'expect_or_drop', 'expect_or_fail']
                if expectation_type not in valid_types:
                    errors.append(f"Expectation {i}: Invalid type '{expectation_type}'. Must be one of: {valid_types}")
            
            # Validate constraint is a valid SQL expression
            constraint = expectation.get('constraint') or expectation.get('expression') or ''
            if constraint and not self._is_valid_sql_constraint(constraint):
                self.logger.warning(f"Expectation {i}: Constraint may not be valid SQL: {constraint}")
    ...
    def _is_valid_sql_constraint(self, constraint: str) -> bool:
        ...
        sql_keywords = ['=', '>', '<', '!=', '<>', 'IS', 'NOT', 'NULL', 'LIKE', 'IN', 'BETWEEN']
        return any(keyword in constraint.upper() for keyword in sql_keywords)
    
    def generate_expectations_template(self, table_name: str, columns: List[str]) -> Dict:
        """Generate template expectations for a table.
        ...
        Returns:
            Dictionary with template expectations
        """
        expectations = {
            'table': table_name,
            'expectations': []
        ...
        # Add common expectations for each column
        for column in columns:
            # Not null check
            expectations['expectations'].append({
                'type': 'expect',
                'constraint': f'{column} IS NOT NULL',
                'message': f'Column {column} should not be null'
            })
            
            # Add type-specific checks based on column name patterns
            if 'id' in column.lower():
                expectations['expectations'].append({
                    'type': 'expect',
                    'constraint': f'{column} > 0',
                    'message': f'ID column {column} should be positive'
                })
            elif 'email' in column.lower():
                expectations['expectations'].append({
                    'type': 'expect_or_drop',
                    'constraint': f"{column} LIKE '%@%'",
                    'message': f'Email column {column} should contain @ symbol'
                })
            elif 'date' in column.lower() or 'timestamp' in column.lower():
                expectations['expectations'].append({
                    'type': 'expect',
                    'constraint': f'{column} IS NOT NULL',
                    'message': f'Date column {column} should not be null'
                })
