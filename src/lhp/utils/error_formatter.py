...
class LHPError(Exception):
    ...
    def __init__(
        ...
        self.doc_link = doc_link or f"https://docs.lakehouseplumber.com/errors/{self.code.lower()}"
...
Option 2 (Legacy - will be deprecated):
  {old_field}: "value" """)
        ...
        details = f"You have specified the same configuration in multiple ways:\n" + "\n".join(conflicts)
        ...
        if preset_name:
            suggestions.append(f"Check if this option is already defined in preset '{preset_name}'")
        ...
        return LHPError(
            ...
            example="\n\n".join(examples),
            context={"Action": action_name, "Preset": preset_name} if preset_name else {"Action": action_name}
        )
    ...
    def validation_errors(
        ...
        for error in errors:
            ...
            if "Missing source" in error:
                ...
                suggestions.append("Add a 'source' field pointing to a view or configuration")
            elif "Invalid target" in error:
                error_details.append("✗ Invalid target reference")
                suggestions.append("Ensure 'target' references a defined view or valid table")
        ...
        return LHPError(
            ...
            details="\n".join(error_details),
            suggestions=suggestions,
            example=f"""Example valid configuration:
actions:
  - name: process_data
    type: transform
    sub_type: sql
...
2. Use intermediate materialization:
   # Create a materialized view at one point in the chain""",
            context={
                "Cycle": cycle_visual,
                "Components": ", ".join(cycle_components)
            }
        ) 