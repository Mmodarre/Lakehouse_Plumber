...
from ..models.config import (
    FlowGroup, Action, ActionType, 
    MetadataColumnConfig, ProjectOperationalMetadataConfig
)
...
class ImportDetector:
    ...
    def _detect_imports_ast(self, expression: str) -> Set[str]:
        ...
        try:
            ...
            self.logger.debug(f"AST parsing failed for expression '{expression}': {e}. Using regex fallback.")
            return self._detect_imports_regex(expression)
...
class OperationalMetadata:
    ...
    def __init__(self, project_config: Optional[ProjectOperationalMetadataConfig] = None):
        self.logger = logging.getLogger(__name__)
    ...
    def resolve_metadata_selection(
        ...
        if hasattr(action, 'operational_metadata') and action.operational_metadata is False:
    ...
    def _extract_column_names(self, selection: Union[bool, List[str]], context: str = "metadata") -> Set[str]:
