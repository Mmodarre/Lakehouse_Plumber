...
class SQLParser:
    ...
    def extract_tables_from_sql(self, sql_content: str) -> List[str]:
        ...
        Handles FROM/JOIN clauses, stream() wrappers, CTEs (CTE names excluded),
        subqueries, set operations, and substitution tokens like ``{catalog}.{schema}``.
        """
    ...
    def _extract_from_from_clauses(
        ...
        for match in re.finditer(from_pattern, sql_content, re.IGNORECASE):
            ...
            # Skip function wrappers — followed by opening parenthesis
    ...
    def _extract_from_ctes(self, sql_content: str) -> tuple[Set[str], Set[str]]:
        """Extract table references from CTE clauses and return CTE names.

        Resolves CTE names incrementally so that a later CTE referencing an earlier one
        is not flagged as an external table.
        """
        tables = set()
        cte_names = set()
