...
class ProgressFormatter:
    ...
    @staticmethod
    def format_pipeline_progress(pipeline_name: str, status: ProgressStatus, details: str = None) -> str:
    ...
    @staticmethod
    def format_flowgroup_progress(flowgroup_name: str, status: ProgressStatus, indent: int = 1) -> str:
        """Format flowgroup progress line with tree structure."""
        prefix = "   " * indent + "├─ "
        return f"{prefix}{flowgroup_name} {status.value}"
    
    @staticmethod
    def format_final_flowgroup(flowgroup_name: str, status: ProgressStatus, details: str = None) -> str:
...
class ErrorHandler:
    ...
    def _format_error_box(self, title: str, content: List[str], error_code: str = None) -> str:
        ...
        # Top border
        lines.append("╭─" + " " + title + " " + "─" * (box_width - len(title) - 4) + "╮")
        ...
        for line in content:
            if len(line) <= box_width - 4:
                ...
                lines.append("│" + " " * (padding + 1) + line + " " * (box_width - 3 - padding - len(line)) + "│")
            else:
    ...
    def handle_cli_error(self, e: Exception, operation: str, show_logs_hint: bool = True) -> None:
        ...
        if isinstance(e, LHPError):
            ...
            if "❌ Error [LHP-" in error_message and "======" in error_message:
                ...
                if self.verbose:
                    ...
                    self.logger.debug(f"{operation} failed: Configuration validation error")
            else:
                ...
                if self.verbose:
                    ...
                    click.echo(f"🔍 Full error details logged")
                else:
    ...
    def _display_formatted_error(self, error_message: str, operation: str) -> None:
        ...
        if 'action' in details and 'unknown_fields' in details:
            if 'section' in details:
                content.append(f"Action '{details['action']}' has unknown field '{details['unknown_fields']}' in {details['section']}")
            else:
                content.append(f"Action '{details['action']}' has unknown field: '{details['unknown_fields']}'")
        ...
        if 'unknown_fields' in details:
            content.append(f"   • Remove the '{details['unknown_fields']}' field from your configuration")
            if 'section' in details:
                content.append(f"   • Check valid fields for {details['section']} in documentation")
            else:
    ...
    def display_pipeline_progress(self, pipeline_name: str, status: ProgressStatus, details: str = None) -> None:
        """Display pipeline progress with consistent formatting."""
        progress_line = self.progress_formatter.format_pipeline_progress(pipeline_name, status, details)
        click.echo(progress_line)
    
    def display_flowgroup_progress(self, flowgroup_name: str, status: ProgressStatus, is_final: bool = False) -> None:
        """Display flowgroup progress with tree structure."""
        if is_final:
            progress_line = self.progress_formatter.format_final_flowgroup(flowgroup_name, status)
        else:
            progress_line = self.progress_formatter.format_flowgroup_progress(flowgroup_name, status)
        click.echo(progress_line)
    ...
    def handle_generation_error(self, e: Exception, action_name: str) -> LHPError:
        ...
        if isinstance(e, FileNotFoundError):
            return LHPError(
                ...
                context=self.context.add_extra("Error Type", "FileNotFoundError").to_dict()
            )
    ...
    def handle_file_error(self, e: Exception, file_path: str, operation: str) -> LHPError:
...
actions:
  - name: my_action
    ...
    def create_config_conflict_error(self, action_name: str, field_pairs: List[tuple], preset_name: Optional[str] = None) -> LHPError:
        """Create a configuration conflict error."""
        return ErrorFormatter.configuration_conflict(action_name, field_pairs, preset_name)
    
    def create_unknown_type_error(self, value_type: str, provided_value: str, valid_values: List[str], example_usage: str) -> LHPError:
        """Create an unknown type error with suggestions."""
        return ErrorFormatter.unknown_type_with_suggestion(value_type, provided_value, valid_values, example_usage)
