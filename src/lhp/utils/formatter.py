...
class CodeFormatter:
    ...
    def _sort_imports(self, import_lines: List[str]) -> List[str]:
        ...
        stdlib_modules = {
            'os', 'sys', 'datetime', 'json', 'logging', 'pathlib',
    ...
    def format_sql(self, sql: str, indent: int = 4) -> str:
        ...
        keywords = [
            ...
            'HAVING', 'UNION', 'LIMIT', 'OFFSET', 'WITH', 'AS'
        ]
        
        formatted = sql
        
        # Add newlines before keywords (handle multi-word keywords first)
        multi_keywords = ['LEFT JOIN', 'RIGHT JOIN', 'INNER JOIN', 'OUTER JOIN', 'GROUP BY', 'ORDER BY']
        ...
        single_keywords = ['FROM', 'WHERE', 'JOIN', 'ON', 'HAVING', 'UNION', 'LIMIT', 'OFFSET', 'WITH', 'AS']
