...
class DataQualityTransformGenerator(BaseActionGenerator):
    ...
    def generate(self, action: Action, flowgroup_config: Dict[str, Any]) -> str:
        ...
        if readMode != "stream":
            raise ValueError(f"Data quality action '{action.name}' requires readMode='stream', got '{readMode}'")
        
        # Read expectations from file
        expectations_file = action.expectations_file
        if not expectations_file:
            raise ValueError(f"Data quality transform '{action.name}' requires expectations_file")
        ...
        if expectations and isinstance(expectations, list):
            ...
            expect_all, expect_all_or_drop, expect_all_or_fail = self.dqe_parser.parse_expectations(expectations)
        ...
        template_context = {
            ...
            "description": action.description or f"Data quality checks for {action.source}"
        }
