...
class CloudFilesLoadGenerator(BaseActionGenerator):
    ...
    def __init__(self):
        ...
        self.known_cloudfiles_options = {
            'format', 'schemaLocation', 'inferColumnTypes', 'maxFilesPerTrigger',
            'maxBytesPerTrigger', 'schemaEvolutionMode', 'rescueDataColumn',
            'includeExistingFiles', 'partitionColumns', 'schemaHints', 'allowOverwrites',
            'backfillInterval', 'cleanSource', 'cleanSource.retentionDuration',
            'cleanSource.moveDestination', 'maxFileAge', 'useIncrementalListing',
            'fetchParallelism', 'pathRewrites', 'resourceTag', 'useManagedFileEvents',
            'useNotifications', 'validateOptions', 'useStrictGlobber'
        }
    ...
    def generate(self, action: Action, context: Dict[str, Any]) -> str:
        ...
        if readMode != "stream":
            raise ValueError(f"CloudFiles action '{action.name}' requires readMode='stream', got '{readMode}'")
        ...
        for legacy_key, cloudfiles_option in legacy_mappings.items():
            if source_config.get(legacy_key) is not None:
                if cloudfiles_option not in reader_options:  # Don't override new options
        ...
        schema_hints_lines = []
        if "cloudFiles.schemaHints" in reader_options:
            schema_hints_variable, schema_hints_lines = self._create_schema_hints_variable(
    ...
    def _process_options(self, options: Dict[str, Any], action_name: str, spec_dir: Path = None) -> Dict[str, Any]:
        ...
        for key, value in options.items():
            ...
            if not key.startswith("cloudFiles.") and key in self.known_cloudfiles_options:
    ...
    def _process_schema_file(self, schema_file_path: str, spec_dir: Path = None) -> Tuple[str, List[str]]:
        ...
        """
        try:
            schema_data = self.schema_parser.parse_schema_file(Path(schema_file_path), spec_dir)
            ...
            variable_name, code_lines = self.schema_parser.to_struct_type_code(schema_data)
            ...
            schema_def_lines = [line for line in code_lines if not line.startswith("from pyspark.sql.types import") and line.strip()]
            ...
            if schema_file_path.startswith('/'):
                ...
                search_locations.append(f"Relative to YAML: {spec_dir / schema_file_path}")
                search_locations.append(f"Project root: {Path.cwd() / schema_file_path}")
    ...
    def _check_conflicts(self, source_config: Dict[str, Any], action_name: str):
        ...
        if len(schema_sources) > 1:
            conflicts.append(f"Multiple schema sources specified: {', '.join(schema_sources)}")
    ...
    def _create_schema_hints_variable(self, schema_hints: str, target_view: str) -> Tuple[str, List[str]]:
