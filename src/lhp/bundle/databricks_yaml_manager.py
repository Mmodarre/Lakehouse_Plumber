...
class DatabricksYAMLManager:
    ...
    def validate_targets_exist(self, environments: List[str]) -> None:
        ...
        if not self.databricks_file.exists():
            raise create_missing_databricks_file_error(str(self.databricks_file))
    ...
    def update_target_variables(
        self, target_environments: List[str], variables: Dict[str, str]
    ) -> None:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Updated databricks.yml variables for targets: {target_environments}"
            )
            ...
            raise BundleResourceError(
                f"Unexpected error updating databricks.yml: {e}"
            ) from e
    ...
    def bulk_update_all_targets(
        self, environments: List[str], environment_variables: Dict[str, Dict[str, str]]
    ) -> None:
        """
        ...
        try:
            ...
            for env in environments:
                ...
                if env in environment_variables:
                    ...
                    self.logger.debug(
                        f"Updated target '{env}' with variables: {environment_variables[env]}"
                    )
            ...
            self.logger.info(
                f"Bulk updated variables in {len(environments)} targets: {environments}"
            )
