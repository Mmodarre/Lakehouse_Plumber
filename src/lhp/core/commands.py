...
class GeneratePipelineCommand(Command):
    ...
    def validate(self, context: CommandContext) -> bool:
        ...
        return (
            context.orchestrator is not None
            and context.project_root is not None
            and context.env is not None
            and context.get_parameter("pipeline_identifier") is not None
        )
    ...
    def execute(self, context: CommandContext) -> ValidationCommandResult:
        ...
        try:
            ...
            errors, warnings = context.orchestrator.validate_pipeline_by_field(
                pipeline_field=pipeline_identifier, env=context.env
            )
...
class AnalyzeStalenessCommand(Command):
    ...
    def execute(self, context: CommandContext) -> AnalysisCommandResult:
        ...
        try:
            ...
            return AnalysisCommandResult(
                success=False, error=str(e), generation_analysis=None
            )
    ...
    def validate(self, context: CommandContext) -> bool:
        ...
        return (
            ...
            and context.project_root is not None
            and context.env is not None
            and bool(context.get_parameter("pipeline_names"))
        )
...
class CommandRegistry:
    ...
    def execute_command(
        self, command_type: str, context: CommandContext
    ) -> CommandResult:
        """
        ...
        if not command.validate(context):
            return CommandResult(
                success=False, error=f"Command {command_type} validation failed"
            )
