...
# against the union of Lakeflow-supported keys;
# (2) `CdcConfigValidator` (L129-330) — validates `cdc_config` for
# `cdc` write-action mode (keys, sequence_by, scd_type, except_column_list,
# track_history_column_list, apply_as_deletes/truncates);
# (3) `SnapshotCdcConfigValidator` (L332-475) — validates `cdc_config` for
# `snapshot_cdc` mode (source_function vs source vs query, snapshot_keys,
# version_column, scd_type compatibility with snapshot inputs);
# (4) `CdcSchemaValidator` (L477-end) — validates the `cdc_schema` block
# (column_list, schema_evolution_mode) and its cross-references to (2)/(3).
# All four share the `Action` shape from `lhp.models.config` (each takes the
# same action argument), the same DLT compatibility constants (the closed set
# of supported table-property keys), and the same error-formatting idiom
# (one validator emits errors keyed by the same action path so callers can
# union the result lists without re-keying). Splitting them across 4 files
