...
class TransformActionValidator(BaseActionValidator):
    ...
    def _validate_data_quality_transform(
        ...
        # Quarantine-specific validation
        if dq_mode == "quarantine":
            ...
            # If mode is dqe or omitted, quarantine block should not be present
            quarantine_config = getattr(action, "quarantine", None)
    ...
    def _validate_quarantine_expectations(
        ...
        Loads the expectations file and checks:
        - File exists and is loadable
        - At least one expectation is present
        - Warns on fail/warn expectations (coerced to drop in quarantine)
        """
        ...
        # Parse all as drop to check count
        parser = DQEParser()
        ...
        # Warn on fail and warn expectations (they will be coerced to drop)
    ...
    def _validate_metadata_columns_not_manipulated(
        self, action: Action, prefix: str, project_config
    ) -> List[str]:
    ...
    def _validate_schema_transform(self, action: Action, prefix: str) -> List[str]:
        ...
        has_schema_file = (
            hasattr(action, "schema_file") and action.schema_file is not None
        )
