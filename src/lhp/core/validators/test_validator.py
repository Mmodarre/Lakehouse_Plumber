...
class TestActionValidator:
    ...
    def _validate_test_type_requirements(
        self, action: Action, prefix: str, test_type: str
    ) -> List[str]:
        ...
        if test_type == "row_count":
            ...
            if not action.source:
                errors.append(
                    f"{prefix}: All lookups found test requires 'source' field"
                )
            ...
            if not action.source:
                errors.append(
                    f"{prefix}: Custom expectations test requires 'source' field"
                )
