...
Or reference it in another transform:
  ...
  - name: further_transform
    ...
    def _build_dependency_graph(self, actions: List[Action]) -> Tuple[Dict[str, List[str]], Dict[str, Action]]:
    ...
    def _topological_sort(self, actions: List[Action], graph: Dict[str, List[str]], targets: Dict[str, Action]) -> List[Action]:
    ...
    def _find_orphaned_actions(self, actions: List[Action], graph: Dict[str, List[str]], targets: Dict[str, Action]) -> List[Action]:
        ...
        for action in actions:
            ...
            if not has_dependents and not has_dependencies and action.type != ActionType.WRITE:
