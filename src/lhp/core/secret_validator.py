...
class SecretValidator:
    ...
    def validate_secret_references(self, secret_refs: Set[SecretReference]) -> List[str]:
        ...
        for secret_ref in secret_refs:
            ...
            if self.available_scopes and secret_ref.scope not in self.available_scopes:
                errors.append(f"Secret scope '{secret_ref.scope}' not found in available scopes")
            
            # Check key format
            if not self._is_valid_key_format(secret_ref.key):
                errors.append(f"Invalid secret key format: '{secret_ref.key}' (must contain only alphanumeric, underscore, or hyphen)")
