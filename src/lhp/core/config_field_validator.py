...
class ConfigFieldValidator:
    ...
    def __init__(self):
        ...
        self.load_source_fields = {
            "cloudfiles": {
                "type", "path", "format", "options", "reader_options", "format_options",
                "schema", "schema_file", "readMode", "schema_location", "schema_infer_column_types",
                "max_files_per_trigger", "schema_evolution_mode", "rescue_data_column"
            },
            "delta": {
                "type", "path", "table", "catalog", "database", "readMode", 
                "cdf_enabled", "read_change_feed", "cdc_options", "where_clause",
                "select_columns", "reader_options"
            },
            "sql": {
                "type", "sql", "sql_path"
            },
            "jdbc": {
                "type", "url", "user", "password", "driver", "query", "table"
            },
            "python": {
                "type", "module_path", "function_name", "parameters"
            }
        }
        ...
        self.write_target_fields = {
            "streaming_table": {
                "type", "database", "table", "create_table", "comment", "description", "table_properties",
                "partition_columns", "cluster_columns", "spark_conf", "schema", "row_filter",
                "temporary", "path", "mode", "cdc_config", "snapshot_cdc_config"
            },
            "materialized_view": {
                "type", "database", "table", "create_table", "comment", "description", "table_properties", 
                "partition_columns", "cluster_columns", "spark_conf", "schema", "row_filter",
                "temporary", "path", "refresh_schedule", "sql"
            }
        }
        
        # Action-level fields that are always valid
        self.action_fields = {
            "name", "type", "source", "target", "description", "readMode", "write_target",
            "transform_type", "sql", "sql_path", "operational_metadata", "expectations_file",
            "once"
        }
    
    def validate_load_source(self, source_config: Dict[str, Any], action_name: str) -> None:
    ...
    def validate_write_target(self, write_target: Dict[str, Any], action_name: str) -> None:
    ...
    def validate_action_fields(self, action_dict: Dict[str, Any], action_name: str) -> None:
    ...
    def _raise_unknown_fields_error(
        ...
        expected_list = sorted(list(expected_fields))
        ...
        for unknown_field in unknown_list:
            if unknown_field == "mode":
                ...
                suggestions.append(f"'{unknown_field}' → use 'type' field in {config_section}")
            else:
        ...
        example = "\n".join(example_lines) if example_lines else "Check field names in documentation"
    ...
    def _find_best_match(self, unknown_field: str, expected_fields: Set[str]) -> Optional[str]:
