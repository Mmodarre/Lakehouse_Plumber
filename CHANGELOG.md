...
tool: `ruff` (v0.13.3) is now the linter, formatter, and import-sorter for the
repo's own code (`src/` + `tests/`). `.flake8` is removed and CI, the
pre-commit config, the Claude Code hooks, and the `dev` dependency group are all
updated to the one-tool flow. The three constitution rules previously enforced
via the standalone B904 / TRY400 / RUF013 selects (§7.3 raise-from, §7.2
`logger.exception`-in-except, §7.4 implicit-`Optional`) remain enforced — now
through the broader `B` / `TRY` / `RUF` rule families. C901 cyclomatic
complexity is enforced at `max-complexity = 40` as a ratchet (see *Deferred*).
This consolidation is scoped to the repo's own tooling and is unrelated to the
generated-output formatting pass (`core/codegen/formatter.py`), which still runs
`ruff format` inside the user's project with its own explicit config so
generated bytes stay deterministic.

**Changed.**

- **`ruff` is the sole linter + formatter + import-sorter for `src/` + `tests/`.**
  The selected lint families are `E`, `W`, `F`, `C901`, `I`, `B`, `C4`, `RET`,
  `PIE`, `RUF`, `TRY` (`[tool.ruff.lint]` in `pyproject.toml`). Import sorting
  (`I`, replacing `isort`) treats `lhp` as first-party; formatting (replacing
  `black`) keeps the prior double-quote / 88-column / magic-trailing-comma style.
- **CI, pre-commit, Claude hooks, and dev-deps migrated to the one-tool flow.**
  `.github/workflows/python_ci.yml` runs `ruff check` + `ruff format --check`;
  `.pre-commit-config.yaml` uses `ruff` + `ruff-format` (the `black` / `flake8` /
  `isort` repos are gone); the Claude Code edit hook auto-fixes via
  `ruff check --fix` / `ruff format`; and the `dev` extra drops `black` /
  `flake8` / `isort` in favour of `ruff>=0.13.3`.
- **Constitution rules B904 / TRY400 / RUF013 are now enforced via families.**
  §7.3 (`B904` raise-from), §7.2 (`TRY400` `logger.exception` in except blocks),
  and §7.4 (`RUF013` PEP 484 implicit-`Optional`) are subsumed by the selected
  `B` / `TRY` / `RUF` families rather than named one-off selects.
- **C901 complexity enforced at `max-complexity = 40` (ratchet).** Starting the
  bar at 40 leaves the tree green immediately — the current worst function,
  `validate_test_type_requirements`, measures exactly 40 — while establishing a
  gate to tighten over subsequent PRs (see *Deferred*).

**Removed.**

- **`.flake8` deleted.** Its configuration is superseded by `[tool.ruff]` /
  `[tool.ruff.lint]` in `pyproject.toml`; `flake8` is no longer a dev dependency.

**Deferred (decided; tracked for follow-up).**

- **`RUFF-PYUPGRADE-DEFER`** — the `UP` / pyupgrade family is deferred to a future
  phase (~2,200 annotation-modernization changes, including the `mypy --strict`
  public-API surface in `lhp/api/`); too broad to land safely in this PR.
- **`RUFF-C901-RATCHET-DEFER`** — `C901` complexity is a ratchet: it starts at
  `max-complexity = 40` (today's worst function `validate_test_type_requirements`
  = 40, so the tree is green on day one) with a roadmap to tighten
  40 → 19 → 14 → 12 → 11 → 10 over subsequent PRs as functions are decomposed.
- **`RUFF-TEST-F841-DEFER`** — test-level `F841` (unused variables; ~44
  occurrences, mostly assigned-but-never-asserted "green lies") is deferred to the
  test-audit and suppressed via the `tests/**` per-file-ignore; deleting them
  blanket-style would violate the no-silent-test-fixes rule, so each is to be
  resolved deliberately under the audit. (`B017` and `RUF043` were resolved
  directly in this PR — the test-residual pass narrowed `pytest.raises(Exception)`
  and raw-string-normalized the `match=` patterns — not deferred.)
- **`RUFF-TIER2-DEFER`** — the Tier-2 rule families (`SIM`, `PERF`, `T201`,
  `PL*`, `RUF059`, `RUF012`) are deferred to a later phase.
